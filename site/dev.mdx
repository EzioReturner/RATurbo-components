...
```bash
$ git clone http://git.luckincoffee.com/front/turbo-components
$ cd turbo-components
$ yarn install
$ yarn dev
```
...
默认文档构建输出目录为 turbo-components/public ，将 public 文件替换至 [luckyturbodocs](http://git.luckincoffee.com/front/luckyturbodocs) 项目 `app/public`，
提交后在 [lsop](http://lsop.luckincoffee.com/base/uda?child_route=3fxhxkx12x8) 中打包部署。
...
$ npm set registry http://verdaccio.luckincoffee.com
$ npm run pub
```